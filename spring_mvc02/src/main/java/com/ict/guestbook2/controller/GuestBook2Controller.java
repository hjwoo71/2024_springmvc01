...
public class GuestBook2Controller {
	...
	public ModelAndView guestBook2WriteOk(GuestBook2VO gb2vo , HttpServletRequest request) {
		...
		try {
			...
			// 파일 정보 
			//System.out.println( "파일정보 : " + gb2vo.getGb2_file_name().getOriginalFilename());
			// summernote 정보까지 
			//System.out.println("써머노트 정보 : " + gb2vo.getGb2_content());
			
			String path = request.getSession().getServletContext().getRealPath("resources/upload");
			MultipartFile file = gb2vo.getGb2_f_name();
			
			if(file.isEmpty()) {
				gb2vo.setGb2_file_name("");
			}else {
				UUID uuid = UUID.randomUUID();
				String f_name = uuid.toString() + "_" +file.getOriginalFilename();
				gb2vo.setGb2_file_name(f_name);
				//파일 업로드
		...
		@PostMapping("/gb2_update_ok")             // jsp갈때 가지고 감  , request는 여기서 쓰고 끝남
		public ModelAndView guestbook2UpdateOK(GuestBook2VO gb2vo , HttpServletRequest request) {
			...
			if(passwordEncoder.matches(pw, pw2)) {
				try {
					...
					// 첨부파일 유무
					if(file.isEmpty()) {
						gb2vo.setGb2_file_name(old_f_name);
					}else {
						UUID uuid = UUID.randomUUID();
						String f_name = uuid.toString() + "_" + file.getOriginalFilename();
						gb2vo.setGb2_file_name(f_name);
