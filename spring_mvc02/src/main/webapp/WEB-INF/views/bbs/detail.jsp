...
<body>
	...
	<div style="padding :10px; width : 600px; margin : 0 auto ">
		<form action="/comment_insert" method="post">
			<fieldset>
				<p> 이름 : <input tyhpe="text" name="writer" > </p>
				<p> 이름 : <textarea rows="3" cols="40" name="content" ></textarea> </p>
	...
	<div style="display : table; margin : 0 auto;">
		...
		<div style="border : 1px solid #cc00cc; width:600px; margin:5px; padding:5px;">
			<form action="/comment_delete" method="post">
		     	<p>이름 : ${k.writer}</p>
		     	<p>내용 : <pre>${k.content}</pre></p>
		     	<p>날짜 : ${k.write_date}</p>
		     	<%-- 실제는 로그인 성공후 관리자이거나 글쓴 본인인 경우만 삭제 가능 --%>
		     	<input type="submit" value="댓글삭제"> 	
		     	<%-- 컨트롤러로 가서 DB 삭제후 다시 이곳으로 와야 한다.(나중에 ajax로 변경하다) --%>
