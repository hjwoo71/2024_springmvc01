...
public class FileController {
	...
	public ModelAndView fileUp(HttpServletRequest request, HttpServletResponse response) {
		try { 
			...
			// 실전에서는 프로젝트가 아닌 다른곳에 저장 할 수도 있다.(AWS의 S3나 서버의 특정위치)
			...
			MultipartRequest mr = 
					new MultipartRequest(
							               ...
							               path,           // 저장위치 
							               500*1024*1024,  // 한번에 업로드할 수 있는 최대크기 : 500MB ( 1024(KB) * 1024(MB) *1024(GB) *1024(TB) )
							               "utf-8",        // 한글 처리 
							               new DefaultFileRenamePolicy()); // 같은이름의 파일이 있을때 이름뒤에 숫자를 붙여 업로드 한다.
			// 파라미터는 mr로 받아야 한다.(request 대신)
